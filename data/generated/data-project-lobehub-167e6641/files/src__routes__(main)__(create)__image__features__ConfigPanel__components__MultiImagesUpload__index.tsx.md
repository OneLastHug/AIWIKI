# 文件：src/routes/(main)/(create)/image/features/ConfigPanel/components/MultiImagesUpload/index.tsx

## 文件职责
这个文件位于 `src/routes/(main)/(create)/image/features/ConfigPanel/components/MultiImagesUpload`。下面根据源码片段、导入导出和命名推断它在项目中的职责。

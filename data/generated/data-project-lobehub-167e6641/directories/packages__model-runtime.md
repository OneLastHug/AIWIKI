...
## 它负责什么
`packages/model-runtime` 是 LobeHub 代码树中的一个功能区域。下面的说明基于真实目录结构和被选中的源码文件生成，后续 Codex 深度解释会继续补全更细的调用关系。

## 下面有哪些子目录
- `docs`：`packages/model-runtime/docs` 下的子功能区，建议展开继续读。
- `src`：`packages/model-runtime/src` 下的子功能区，建议展开继续读。

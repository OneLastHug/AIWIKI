...
def repo_card_html(row: sqlite3.Row) -> str:
    ...
    ctx = repo_card_context(source, repo_id, str(row["status"] or ""))
    docs_href = f"/repos/{repo_id}/"
    # 跳转图标：仅远程仓库显示，指向源仓库（GitHub/GitLab），新标签打开
    if is_remote(source):
        jump = (
            f"<a class='repo-jump' href='{html.escape(source, quote=True)}' target='_blank' "
            f"rel='noopener noreferrer' title='打开源仓库' aria-label='打开源仓库'>"
            "<svg viewBox='0 0 24 24' width='14' height='14' fill='none' stroke='currentColor' "
            "stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round'>"
            "<line x1='7' y1='17' x2='17' y2='7'></line>"
            "<polyline points='7 7 17 7 17 17'></polyline></svg></a>"
        )
    else:
        jump = ""
    return (
        ...
        f"<a class='repo-title' href='{docs_href}'>{html.escape(ctx['title'])}</a>"
        f"<div class='repo-subtitle'>{html.escape(ctx['subtitle'])}</div>"
        f"</div>"
        f"<div class='repo-footer'>"
        f"<a class='repo-open' href='{docs_href}'>查看文档</a>{jump}"
        f"</div>"
        f"</li>"
    )

...
def repo_card_html(row: sqlite3.Row) -> str:
    ...
    return (
        ...
        f"<div class='repo-footer'><span class='repo-open'>打开学习文档</span>"
        f"<span class='repo-hint'>{html.escape(str(row['repo_id']))}</span></div>"
        f"</a></li>"
    )
...
.home-page{{display:grid;gap:34px}}.home-page p{{text-indent:0!important}}.home-hero{{padding:10px 0 4px}}.home-kicker{{margin:0 0 12px;color:var(--accent);font-size:13px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}}.home-title{{margin:0;font-size:46px;line-height:1.14;letter-spacing:-.03em;color:var(--text)}}.home-subtitle{{max-width:760px;margin:16px 0 0;color:var(--text-2);font-size:17px;line-height:1.9}}.home-section-head{{display:flex;align-items:end;justify-content:space-between;gap:18px;flex-wrap:wrap;margin:0 0 18px}}.home-section-kicker{{margin:0 0 6px;color:var(--muted);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}}.home-section-title{{margin:0;font-size:28px;line-height:1.2;color:var(--text)}}.home-section-note{{max-width:420px;margin:0;color:var(--muted);font-size:14px;line-height:1.7}}.repo-list{{display:grid;gap:18px;list-style:none;padding:0;margin:0}}.repo-grid{{grid-template-columns:repeat(auto-fit,minmax(280px,1fr))}}.repo-card{{border:1px solid color-mix(in srgb,var(--accent) 12%,var(--border));border-radius:22px;background:linear-gradient(180deg,var(--panel) 0,var(--panel-2) 100%);box-shadow:0 18px 40px rgba(15,23,42,.06);overflow:hidden;transition:transform .16s ease, box-shadow .16s ease, border-color .16s ease}}.repo-card:hover{{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 28%,var(--border-2));box-shadow:0 24px 46px rgba(15,23,42,.1)}}.repo-card-link{{display:grid;gap:14px;padding:22px 22px 20px;color:inherit}}.repo-card-top{{display:flex;align-items:center;justify-content:space-between;gap:12px}}.repo-kicker{{display:inline-flex;align-items:center;padding:5px 10px;border-radius:999px;background:color-mix(in srgb,var(--accent) 10%,transparent);color:var(--accent);font-size:12px;font-weight:780;letter-spacing:.04em;text-transform:uppercase}}.repo-status{{display:inline-flex;align-items:center;padding:5px 10px;border-radius:999px;background:var(--soft);border:1px solid var(--border);color:var(--text-2);font-size:12px;font-weight:700}}.repo-title{{font-size:28px;line-height:1.08;font-weight:820;color:var(--text);letter-spacing:-.03em;word-break:break-word}}.repo-subtitle{{color:var(--text-2);font-size:15px;font-weight:650;line-height:1.55;word-break:break-word}}.repo-meta{{color:var(--muted);font-size:13px;line-height:1.65;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}}.repo-footer{{display:flex;align-items:center;justify-content:space-between;gap:12px;padding-top:10px;border-top:1px solid var(--border)}}.repo-open{{color:var(--accent);font-size:14px;font-weight:760}}.repo-hint{{color:var(--muted);font-size:11px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:46%}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/":
            ...
            self.send_html("Repo Docs", f"<div class='home-page'><section class='home-hero'><p class='home-kicker'>AIWIKI</p><h1 class='home-title'>项目名先看清，再决定读哪个仓库。</h1><p class='home-subtitle'>输入 GitHub/GitLab HTTPS URL 或 /data/project 下本地仓库，系统会过滤纯 MD/PDF/Office/TXT 文档仓库，只保留真正适合代码学习的项目，并自动生成分段中文讲解。</p><form class='source-form' method='post' action='/submit'><input name='source' aria-label='仓库地址或本地路径' placeholder='https://github.com/org/repo 或 /data/project/lobehub'><button type='submit'>生成学习文档</button></form></section><section><div class='home-section-head'><div><p class='home-section-kicker'>Project Library</p><div class='home-section-title'>已有项目</div></div><p class='home-section-note'>卡片主标题只显示真正的项目名，来源和状态放在副信息里，不再让长串 repo_id 抢视线。</p></div><ul class='repo-list repo-grid'>{repo_html}</ul></section></div>") ; return
